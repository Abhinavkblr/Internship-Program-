...
--Use a subquery to find the insurance plan that was claimed the most for each disease.
--Join the InsurancePlan, Claim, Treatment, and Disease tables to connect the plan information with the disease and claim count.
--Group the results by the plan name and order by the claim count in descending order to get the maximum claimed plan.
--Use another subquery to find the insurance plan that was claimed the least for each disease, following a similar approach but ordering by the claim count in ascending order.
--Select the disease name, maximum claimed plan, and minimum claimed plan for each disease.
